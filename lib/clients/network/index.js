...
function NetworkService() {
	...
	self.MobileData = {
		get: (partner_script_id) => {
			return self.get('MobileData?partner_script_id=' + partner_script_id);
		}
	};

	self.ClientToken = {
		post: (application_id, partner_script_id, provider, oauth_key, oauth_secret, token_data) => {
	...
	self.CommitClientJob = {
		post: (application_id, partner_script_id, profile_ids, verification_data) => {
