...
function Authentication(id, secret) {
	var self = this instanceof Authentication ? this : new Authentication(id, secret);
	 ...
	 * @param url
	 */
	self.getSignature = function(method, body, date, url) {
		// Build the MD5
		var contentMd5 = !!body ? hash.md5(body) : "";
		// Build the string to be signed
		var stringToSign = self.getStringToSign(method, contentMd5, date, url);
 ...
 * @returns {*}
 */
Authentication.prototype.stripQueryString = function(path) {
 ...
 */
Authentication.prototype.getStringToSign = function(method, contentMd5, date, path) {
 ...
 * @returns {string}
 */
Authentication.prototype.signString = function(stringToSign, secret) {
